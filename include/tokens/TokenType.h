...
/**
 * @enum TokenType
 * @brief Represents the different types of tokens in Lax.
 *
 * This enum is used to differentiate between the different types of tokens that
 * can be encountered when parsing a Lax program. These tokens are divided into
 * categories such as operators, identifiers, constants, type names, keywords,
 * punctuation marks, and others.
 *
 * The UNKNOWN token type is used for unrecognized tokens.
 *
 * @note The values assigned to the tokens start from 256 to avoid clashes with
 *       ASCII char values.
 */
enum TokenType {
    UNKNOWN = 256,  // No type
    ...
    SIMEQ = 281,    // Assignment operator (simple equal sign)
