...
ObjFunction* Compiler::compile(AST& ast) {
    ...
    return m_errors ? nullptr : script;
}
...
bool Compiler::writeBytecode(Chunk* chunk) {
    ...
    uint8_t bytes[] = {
        static_cast<uint8_t>(constantCount >> 8),
        static_cast<uint8_t>(constantCount & 0xff)
    };
...
Chunk* Compiler::currentChunk() {
    return m_currentScope->function->getChunk();
}
...
void Compiler::declareVariable(ObjString* name) {
    ...
    if (m_currentScope->depth == 0) return;

    // Check if the variable is not already declared in the current scope
    for (int i = m_currentScope->localCount - 1; i >= 0; --i) {
        Local& local = m_currentScope->locals[i];

        // Don't mind if the variable shadows an outer local variable or an
        // uninitialized variable
        if (local.depth != -1 && local.depth > m_currentScope->depth) break;
...
void Compiler::addLocal(ObjString* name) {
    ...
    if (m_currentScope->localCount == UINT8_MAX) {
        error("Too many local variables in scope");
        return;
    }

    // Add the variable as a local to the array
    Local& local = m_currentScope->locals[m_currentScope->localCount++];
    local.name = name;
    local.depth = -1; // Mark uninitialized, so that it cannot be referenced yet
...
void Compiler::markLocalInitialized() {
    ...
    size_t localIndex = m_currentScope->localCount - 1;
    m_currentScope->locals[localIndex].depth = m_currentScope->depth;
}
...
int Compiler::resolveUpvalue(Scope* scope, ObjString* name) {
    ...
    // If found, mark the local as captured, so it can now be considered as an
    // upvalue in the current scope
    if (local != -1) {
    ...
    if (upvalue != -1)
        return addUpvalue(scope, static_cast<uint8_t>(upvalue), false);
...
void Compiler::initScope(Scope* scope, FunctionType type) {
    ...
    scope->enclosing = m_currentScope;
...
void Compiler::visit(AssignNode& node) {
    ...
    else if ((arg = resolveUpvalue(m_currentScope, identifier)) != -1) {
        emitInstruction(OP_SET_UPVALUE, static_cast<uint16_t>(arg));
    }

    // Assume that the variable is global and assign it the value
    // Note that the global variable might not exist depending on the
    // runtime state
    else {
...
void Compiler::visit(ConditionalNode& node) {
    ...
    // Fill the first jump so that it knows where to jump to overfly the
    // then branch
    patchJump(thenJump);

    // Pop the condition result value from the runtime stack (the previous pop
    // didn't occur)
    emitByte(OP_POP);
    ...
    // Fill the first jump so that it knows where to jump to overfly the else
    // branch
    patchJump(elseJump);
...
void Compiler::visit(LogicalNode& node) {
    ...
    switch (operatorType) {
        case TokenType::AND: {
            ...
            //
            // Bytecode in case left operand is true ; will not be executed at
            // runtime if false
            //
            ...
            // Fill the previous jump so that it knows where to jump if the
            // operand was false
            patchJump(endJump);
            // emitByte(OP_POP); // Pop the operand result from the runtime stack
            break;
        ...
        case TokenType::OR: {
            ...
            //
            // Bytecode in case left operand is false ; will not be executed at
            // runtime if true
            //

            // Fill the first jump so that it knows where to jump if the
            // operand was false
            patchJump(elseJump);
            emitByte(OP_POP); // Pop the operand result from the runtime stack
            ...
            // Fill the second jump so that it knows where to jump if the
            // operand was true
            patchJump(endJump);
...
void Compiler::visit(RelationalNode& node) {
    ...
    switch (operatorType) {
        case TokenType::EQ:     emitByte(OP_EQUAL);
            break;
        case TokenType::NEQ:    emitByte(OP_NOT_EQUAL);
            break;
        case TokenType::SL:     emitByte(OP_LESS);
            break;
        case TokenType::LE:     emitByte(OP_LESS_EQUAL);
            break;
        case TokenType::SG:     emitByte(OP_GREATER);
            break;
        case TokenType::GE:     emitByte(OP_GREATER_EQUAL);
            break;
        default: return; // Unreachable
    }
...
void Compiler::visit(FunNode& node) {
    ...
    for (const auto & i : node.getParams()) {
        ObjString* paramIdentifier = copyIdentifier(i->getVarName()->toString());
        declareVariable(paramIdentifier);
        defineVariable(identifierConstant(paramIdentifier));
    }

    scope.function = new ObjFunction(identifier, (int)node.getParams().size());
    ...
    // Emit a closure to capture at runtime all local variables from
    // surrounding scopes
...
void Compiler::visit(ReturnNode& node) {
    ...
    if (m_currentScope->type == FunctionType::TYPE_SCRIPT)
...
void Compiler::visit(BinOpNode& node) {
    ...
    // Emit the operation instruction
    switch (operatorType) {
        case TokenType::PLUS:   emitByte(OP_ADD);
            break;
        case TokenType::MINUS:  emitByte(OP_SUBTRACT);
            break;
        case TokenType::STAR:   emitByte(OP_MULTIPLY);
            break;
        case TokenType::SLASH:  emitByte(OP_DIVIDE);
            break;
        default: return; // Unreachable
    }
...
void Compiler::visit(UnaryNode& node) {
    ...
    // Emit the operation instruction
    switch (operatorType) {
        case TokenType::BANG:   emitByte(OP_NOT);
            break;
        case TokenType::MINUS:  emitByte(OP_NEGATE);
            break;
        default: return; // Unreachable
    }
