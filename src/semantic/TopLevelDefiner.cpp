#include "semantic/TopLevelDefiner.h"
#include "ast/AST.h"
...
TopLevelDefiner::TopLevelDefiner(AST& ast) : SemanticAnalyzer(ast) {}

//void TopLevelDefiner::visit(ExprNode& node) {
//    bindScope(node); // Bind scope for all StmtNode and subclasses
//    SemanticAnalyzer::visit(node);
//}

//void TopLevelDefiner::visit(AssignNode& node) {
...
//void TopLevelDefiner::visit(CallNode& node) {
...
//void TopLevelDefiner::visit(IdNode& node) {
...
void TopLevelDefiner::visit(StmtNode& node) {
    bindScope(node); // Bind scope for all StmtNode and subclasses
    SemanticAnalyzer::visit(node);
}

//void TopLevelDefiner::visit(BlockNode& node) {
//    m_symtable->beginScope();
//    visit(*node.getSequence());
//    m_symtable->endScope();
//}

void TopLevelDefiner::visit(DeclNode& node) {
    // Don't visit expression assigned
...
void TopLevelDefiner::visit(FunNode& node) {
...
void TopLevelDefiner::bindScope(ASTNode& node) {
    node.setScope(&m_symtable->currentScope());
}

void TopLevelDefiner::visit(StmtExpressionNode& node) {
